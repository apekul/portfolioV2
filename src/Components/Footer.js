...
function Footer() {
  return (
    <footer className=" text-white ">
      <ul
        id="contact"
        className="Container bg-[#4F46E5] flex flex-col md:flex-row items-start md:items-center justify-center gap-10 py-10"
      >
        {contact.map((item, index) => (
          <li key={index} className="flex items-center gap-3">
            ...
            >
              <label
                className={`font-bold text-lg ${item.nav && "cursor-pointer"}`}
              >
                {item.title}
              </label>
              <p>{item.text}</p>
